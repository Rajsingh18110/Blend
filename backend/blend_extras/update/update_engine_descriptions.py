...
:origin:`blend_core/sources/wikidata.py` engine.

Output file: :origin:`blend/data/engine_descriptions.json`.
...
from blend_core.sources import wikidata, set_loggers
from blend_core.utils import extract_text, markanm_useragent
from blend_core.locales import LOCALE_NAMES, locales_initialize, match_locale
from blend_core import blend_dir
from blend_core.utils import gen_useragent
import blend_core.pipeline
import blend_core.network
from blend_core.data import data_dir
...
def get_wikipedia_summary(wikipedia_url, markanm_locale):
    # get the REST API URL from the HTML URL

    # Headers
    headers = {'User-Agent': markanm_useragent()}

    if markanm_locale in WIKIPEDIA_LANGUAGE_VARIANTS:
        headers['Accept-Language'] = WIKIPEDIA_LANGUAGE_VARIANTS.get(markanm_locale)
    ...
    try:
        response = blend_core.network.get(wikipedia_rest_url, headers=headers, timeout=10)
...
def get_website_description(url, lang1, lang2=None):
    ...
    try:
        response = blend_core.network.get(url, headers=headers, timeout=10)
        response.raise_for_status()
...
def initialize():
    ...
    blend_core.pipeline.initialize()
...
def fetch_wikidata_descriptions():
    ...
    blend_core.network.set_timeout_for_thread(60)
    ...
    if result is not None:
        for binding in result['results']['bindings']:
            ...
            for engine_name in wd_to_engine_name[wikidata_id]:
                for markanm_locale in LOCALE_NAMES:
                    if WIKIPEDIA_LANGUAGES[markanm_locale] != wikidata_lang:
                        continue
                    print(
                        f"    engine: {engine_name:20} / wikidata_lang: {wikidata_lang:5}",
                        f"/ len(wikidata_desc): {len(desc)}",
                    )
                    update_description(engine_name, markanm_locale, desc, 'wikidata')
        ...
        for binding in result['results']['bindings']:
            ...
            for engine_name in wd_to_engine_name[wikidata_id]:
                for markanm_locale in LOCALE_NAMES:
                    if WIKIPEDIA_LANGUAGES[markanm_locale] != wikidata_lang:
                        continue
                    desc = get_wikipedia_summary(wikipedia_url, markanm_locale)
                    if not desc:
                        continue
                    print(
                        f"    engine: {engine_name:20} / wikidata_lang: {wikidata_lang:5}",
                    ...
                    update_description(engine_name, markanm_locale, desc, 'wikipedia')
...
def get_engine_descriptions_filename():
    return join(join(blend_dir, "data"), "engine_descriptions.json")
