...
class ProxyFix:
    ...
    .. sidebar:: :py:obj:`flask.Request.remote_addr`
       ...
       Markanm uses Werkzeug's ProxyFix_ (with it default ``x_for=1``).
    ...
    - `X-Real-IP <https://github.com/markanm/markanm/issues/1237#issuecomment-1147564516>`__:
    ...
    def __call__(self, environ: "WSGIEnvironment", start_response: "StartResponse") -> abc.Iterable[bytes]:
        ...
        if not x_forwarded_for and not x_real_ip:
            logger.debug("X-Forwarded-For nor X-Real-IP header is set! (Normal for internal pings)")
