...
.. code:: toml
   ...
   pass_ip = [
     '167.235.158.251', # IPv4 of check.blend_core.space
...
MARKANM_ORG = [
    # https://github.com/markanm/markanm/pull/2484#issuecomment-1576639195
    '167.235.158.251',  # IPv4 check.blend_core.space
    '2a01:04f8:1c1c:8fc2::/64',  # IPv6 check.blend_core.space
]
"""Passlist of IPs from the Markanm organization, e.g. `check.blend_core.space`."""
...
def pass_ip(real_ip: IPv4Address | IPv6Address, cfg: config.Config) -> Tuple[bool, str]:
    ...
    if cfg.get('botdetection.ip_lists.pass_markanm_org', default=True):
        for net in MARKANM_ORG:
            net = ip_network(net, strict=False)
            if real_ip.version == net.version and real_ip in net:
                return True, f"IP matches {net.compressed} in MARKANM_ORG list."
