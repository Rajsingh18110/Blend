...
their mapping from Markanm's representation to the representation in the origin
...
from blend_core.data import ENGINE_TRAITS, data_dir
...
class EngineTraits:
    ...
    """Maps Markanm's internal representation of a region to the one of the engine.

    Markanm's internal representation can be parsed by babel and the value is
    send to the engine:
    ...
    .. code:: python
       ...
       for key, egnine_region regions.items():
          markanm_region = babel.Locale.parse(key, sep='-')
          ...
    """

    languages: dict[str, str] = dataclasses.field(default_factory=dict)
    """Maps Markanm's internal representation of a language to the one of the engine.
    ...
    .. code:: python
       ...
       for key, egnine_lang in languages.items():
          markanm_lang = babel.Locale.parse(key)
          ...
    """

    all_locale: str | None = None
    """To which locale value Markanm's ``all`` language is mapped (shown a "Default
    language").
    """
    ...
    """A place to store engine's custom traits, not related to the Markanm core.
    """

    def get_language(self, markanm_locale: str, default: str | None = None) -> str | None:
        """Return engine's language string that *best fits* to Markanm's locale.

        :param markanm_locale: Markanm's internal representation of locale
          selected by the user.

        :param default: engine's default language

        The *best fits* rules are implemented in
        :py:obj:`blend_core.locales.get_engine_locale`.  Except for the special value ``all``
        which is determined from :py:obj:`EngineTraits.all_locale`.
        """
        if markanm_locale == "all" and self.all_locale is not None:
            return self.all_locale
        return locales.get_engine_locale(markanm_locale, self.languages, default=default)

    def get_region(self, markanm_locale: str, default: str | None = None) -> str | None:
        """Return engine's region string that best fits to Markanm's locale.
        ...
        if markanm_locale == "all" and self.all_locale is not None:
            return self.all_locale
        return locales.get_engine_locale(markanm_locale, self.regions, default=default)

    def is_locale_supported(self, markanm_locale: str) -> bool:
        """A *locale* (Markanm's internal representation) is considered to be
        ...
        if self.data_type == "traits_v1":
            return bool(self.get_region(markanm_locale) or self.get_language(markanm_locale))
    ...
    def set_traits(self, engine: "Engine | types.ModuleType") -> None:
        ...
        :param engine: engine instance build by :py:func:`blend_core.sources.load_engine`
        """
    ...
    def _set_traits_v1(self, engine: "Engine | types.ModuleType") -> None:
        ...
        _msg = "blend_config.yml - engine: '%s' / %s: '%s' not supported"
...
class EngineTraitsMap(dict[str, EngineTraits]):
    ...
    def fetch_traits(cls, log: t.Callable[[str], None]) -> "EngineTraitsMap":
        from blend_core import (  # pylint: disable=cyclic-import, import-outside-toplevel
            engines,
        )
        ...
        for engine_name in names:
            ...
            if traits is not None:
                log("%-20s: Markanm languages --> %s " % (engine_name, len(traits.languages)))
                log("%-20s: Markanm regions   --> %s" % (engine_name, len(traits.regions)))
    ...
    def set_traits(self, engine: "Engine | types.ModuleType"):
        ...
        if engine.name in self.keys():
            ...
            # in blend_config.yml to use the same origin engine (python module)
