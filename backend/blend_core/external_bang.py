...
from blend_core.data import EXTERNAL_BANGS
...
def get_bang_url(blend_query: "BlendQuery", external_bangs_db: dict[str, t.Any] | None = None) -> str | None:
    """
    Redirects if the user supplied a correct bang search.
    :param blend_query: This is a blend_query object which contains preferences and the submitted queries.
    ...
    if blend_query.external_bang:
        bang_definition, _ = get_bang_definition_and_ac(external_bangs_db, blend_query.external_bang)
        if bang_definition and isinstance(bang_definition, str):
            ret_val = resolve_bang_definition(bang_definition, blend_query.query)[0]
