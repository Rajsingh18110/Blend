...
from .blend_locales import blend_locales

blend_dir = abspath(dirname(__file__))
...
def apply_schema(settings: dict[str, t.Any], schema: dict[str, t.Any], path_list: list[str]):
    ...
    if len(path_list) == 0 and error:
        raise ValueError("Invalid blend_config.yml")
    return error


SCHEMA: dict[str, t.Any] = {
    'general': {
        'debug': SettingsValue(bool, False, 'MARKANM_DEBUG'),
        'instance_name': SettingsValue(str, 'Markanm'),
        'privacypolicy_url': SettingsValue((None, False, str), None),
        'contact_url': SettingsValue((None, False, str), None),
        'donation_url': SettingsValue((bool, str), "https://docs.markanm.org/donate.html"),
    ...
    'search': {
        ...
        'suspended_times': {
            'BlendEngineAccessDenied': SettingsValue(numbers.Real, 86400),
            'BlendEngineCaptcha': SettingsValue(numbers.Real, 86400),
            'BlendEngineTooManyRequests': SettingsValue(numbers.Real, 3600),
            'cf_BlendEngineCaptcha': SettingsValue(numbers.Real, 1296000),
            'cf_BlendEngineAccessDenied': SettingsValue(numbers.Real, 86400),
            'recaptcha_BlendEngineCaptcha': SettingsValue(numbers.Real, 604800),
        },
        'formats': SettingsValue(list, OUTPUT_FORMATS),
        'max_page': SettingsValue(int, 0),
    },
    'server': {
        'port': SettingsValue((int, str), 8888, 'MARKANM_PORT'),
        'bind_address': SettingsValue(str, '127.0.0.1', 'MARKANM_BIND_ADDRESS'),
        'limiter': SettingsValue(bool, False, 'MARKANM_LIMITER'),
        'public_instance': SettingsValue(bool, False, 'MARKANM_PUBLIC_INSTANCE'),
        'secret_key': SettingsValue(str, environ_name='MARKANM_SECRET'),
        'base_url': SettingsValue((False, str), False, 'MARKANM_BASE_URL'),
        'image_proxy': SettingsValue(bool, False, 'MARKANM_IMAGE_PROXY'),
        'http_protocol_version': SettingsValue(('1.0', '1.1'), '1.0'),
        'method': SettingsValue(('POST', 'GET'), 'POST', 'MARKANM_METHOD'),
        'default_http_headers': SettingsValue(dict, {}),
    },
    # redis is deprecated ..
    'redis': {
        'url': SettingsValue((None, False, str), False, 'MARKANM_REDIS_URL'),
    },
    'valkey': {
        'url': SettingsValue((None, False, str), False, 'MARKANM_VALKEY_URL'),
    },
    'ui': {
        'static_path': SettingsDirectoryValue(str, os.path.join(blend_dir, 'assets')),
        'templates_path': SettingsDirectoryValue(str, os.path.join(blend_dir, 'views')),
