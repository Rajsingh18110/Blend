...
from blend_core.metrics.error_recorder import count_error
...
class OnlineProcessor(EngineProcessor):
    ...
    def init_network_in_thread(self, start_time: float, timeout_limit: float):
        ...
        blend_core.network.set_timeout_for_thread(timeout_limit, start_time=start_time)
        # reset the HTTP total time
        blend_core.network.reset_time_for_thread()
        # set the network
        blend_core.network.set_context_network_name(self.engine.name)

    def get_params(self, blend_query: "BlendQuery", engine_category: str) -> OnlineParams | None:
        ...
        base_params: RequestParams | None = super().get_params(blend_query, engine_category)
        ...
        if self.engine.send_accept_language_header and blend_query.locale:
            _l = blend_query.locale.language
            _t = blend_query.locale.territory or _l
    ...
    def _send_http_request(self, params: OnlineParams):
        ...
        if params["method"] == "GET":
            req = blend_core.network.get
        else:
            req = blend_core.network.post
            if params["data"]:
    ...
    def blend_search(  # pyright: ignore[reportIncompatibleMethodOverride]
        self,
        query: str,
        params: OnlineParams,
        ...
        try:
            ...
            self.handle_exception(result_pool, e, suspend=True)
            self.logger.error("SSLError {}, verify={}".format(e, blend_core.network.get_network(self.engine.name).verify))
