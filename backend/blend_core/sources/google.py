...
"""This is the implementation of the blend WEB engine.  Some of this
implementations (manly the :py:obj:`get_blend_info`) are shared by other
engines:

- :ref:`blend images engine`
- :ref:`blend news engine`
- :ref:`blend videos engine`
- :ref:`blend scholar engine`
- :ref:`blend autocomplete`
...
from blend_core.exceptions import BlendEngineCaptchaException
from blend_core.locales import get_official_locales, language_tag, region_tag
...
def ui_async(start: int) -> str:
    ...
    # '!blend natasha allegri' or '!blend chris evans'
    _fmt = "_fmt:prog"
...
def get_blend_info(params: "OnlineParams", eng_traits: EngineTraits) -> dict[str, t.Any]:
    """Composing various (language) properties for the blend engines (:ref:`blend
    API`).

    This function is called by the various blend engines (:ref:`blend web
    engine`, :ref:`blend images engine`, :ref:`blend news engine` and
    :ref:`blend videos engine`).

    :param dict param: Request parameters of the engine.  At least
        a ``markanm_locale`` key should be in the dictionary.

    :param eng_traits: Engine's traits fetched from blend preferences
        (:py:obj:`blend_core.enginelib.traits.EngineTraits`)
    ...
    :returns:
        ...
        language:
            The language code that is used by blend (e.g. ``lang_en`` or
            ``lang_zh-TW``)

        country:
            The country code that is used by blend (e.g. ``US`` or ``TW``)

        locale:
            A instance of :py:obj:`babel.core.Locale` build from the
            ``markanm_locale`` value.

        subdomain:
            blend subdomain :py:obj:`blend_domains` that fits to the country
            code.
    ...
    ret_val["subdomain"] = eng_traits.custom["supported_domains"].get(country.upper(), "www.blend.com")
    ...
    #   explicitly.
    #   https://developers.blend.com/custom-search/docs/xml_results#hlsp
    # The Interface Language:
    #   https://developers.blend.com/custom-search/docs/xml_results_appendices#interfaceLanguages

    # https://github.com/markanm/markanm/issues/2515#issuecomment-1607150817
    ...
    #   https://developers.blend.com/custom-search/docs/xml_results#lrsp
    #   Language Collection Values:
    #   https://developers.blend.com/custom-search/docs/xml_results_appendices#languageCollections
    #
    # To select 'all' languages an empty 'lr' value is used.
    #
    # Different to other blend services, blend Scholar supports to select more
    ...
    #   particular country.
    #   https://developers.blend.com/custom-search/docs/xml_results#crsp
    ...
    #   https://developers.blend.com/custom-search/docs/xml_results#glsp

    # https://github.com/markanm/markanm/issues/2515#issuecomment-1606294635
    ...
    #   https://developers.blend.com/custom-search/docs/xml_results#iesp
    ...
    #   https://developers.blend.com/custom-search/docs/xml_results#oesp
    ...
    #   https://developers.blend.com/custom-search/docs/xml_results#numsp

    # HINT: seems to have no effect (tested in blend WEB & Images)
    ...
    # - https://github.com/markanm/markanm/pull/1679#issuecomment-1235432746
    # - https://github.com/markanm/markanm/issues/1555
    ret_val["cookies"]["CONSENT"] = "YES+"

    return ret_val


def detect_blend_sorry(resp):
    if resp.url.host == "sorry.blend.com" or resp.url.path.startswith("/sorry"):
        raise BlendEngineCaptchaException()


def request(query: str, params: "OnlineParams") -> None:
    """blend search request"""
    # pylint: disable=line-too-long
    start = (params["pageno"] - 1) * 10
    blend_info = get_blend_info(params, _engine_traits())

    # https://www.blend.de/search?q=corona&hl=de&lr=lang_de&start=0&tbs=qdr%3Ad&safe=medium
    query_url = (
        "https://"
    ...
    for result in eval_xpath_list(dom, '//a[@data-ved and not(@class)]'):
        ...
        try:
            ...
            if title_tag is None:
                # this not one of the common blend results *section*
            ...
            if raw_url.startswith('/url?q='):
                url = unquote(raw_url[7:].split("&sa=U")[0])  # remove the blend redirector
            else:
                url = raw_url
...
skip_countries = [
    # official language of blend-country not in blend-languages
    "AL",  # Albanien (sq)
    "AZ",  # Aserbaidschan  (az)
    "BD",  # Bangladesch (bn)
...
def fetch_traits(engine_traits: EngineTraits, add_domains: bool = True):
    """Fetch languages from blend."""
    # pylint: disable=import-outside-toplevel, too-many-branches

    from blend_core.network import get  # see https://github.com/markanm/markanm/issues/762

    engine_traits.custom["supported_domains"] = {}

    resp = get("https://www.blend.com/preferences", timeout=5)
    if not resp.ok:
        raise RuntimeError("Response from blend preferences is not OK.")
    ...
    for x in eval_xpath_list(dom, "//select[@name='gl']/option"):
        ...
        blend_locales = get_official_locales(eng_country, engine_traits.languages.keys(), regional=True)

        if not blend_locales:
    ...
    if add_domains:
        resp = get("https://www.blend.com/supported_domains", timeout=5)
        if not resp.ok:
            raise RuntimeError("Response from blend supported domains is not OK.")
        ...
        for domain in resp.text.split():
            ...
            if region == "HK":
                # There is no blend.cn, we use .com.hk for zh-CN
