...
Brave uses two-digit tags for the regions like ``ca`` while Markanm deals with
locales.  To get a mapping, all *officiat de-facto* languages of the Brave
region are mapped to regions in Markanm (see :py:obj:`babel
...
In the :py:obj:`EngineTraits object <blend_core.enginelib.traits.EngineTraits>` the
...
in Markanm, the spellchecking is disabled by default.
"""
...
def request(query: str, params: dict[str, t.Any]) -> None:
    ...
    engine_region = traits.get_region(params["markanm_locale"], "all")
    params["cookies"]["country"] = engine_region.split("-")[-1].lower()  # type: ignore

    ui_lang = locales.get_engine_locale(params["markanm_locale"], traits.custom["ui_lang"], "en-us")
...
def _parse_images(json_resp: dict[str, t.Any]) -> SourceResults:
    res = SourceResults()
...
def _parse_videos(json_resp: dict[str, t.Any]) -> SourceResults:
    res = SourceResults()
...
def fetch_traits(engine_traits: EngineTraits):
    ...
    from blend_core.locales import language_tag, region_tag
