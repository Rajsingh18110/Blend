...
:py:obj:`startpage_categ` in  blend_config.yml::
...
from blend_core.locales import region_tag
...
def get_sc_code(params):
    ...
    try:
        ...
        logger.debug("suspend startpage API --> https://github.com/markanm/markanm/pull/695")
        raise BlendEngineCaptchaException(
...
def request(query, params):
    ...
    """
    engine_region = traits.get_region(params["markanm_locale"], "en-US")
    engine_language = traits.get_language(params["markanm_locale"], "en")
...
def response(resp):
    ...
    for results_categ in results_obj.get("mainline", []):
        for item in results_categ.get("results", []):
            if results_categ["display_type"] == "web-blend":
...
def fetch_traits(engine_traits: EngineTraits):
    ...
    skip_eng_tags = {
        "english_uk",  # Markanm lang 'en' already maps to 'english'
    }
