...
With this options a Markanm maintainer is able to configure **additional**
...
from blend_core.data import ENGINE_TRAITS
from blend_core.enginelib.traits import EngineTraits
from blend_core.exceptions import BlendException
from blend_core.result_types import SourceResults
from blend_core.utils import ElementType, eval_xpath, eval_xpath_list, extract_text
...
def request(query: str, params: "OnlineParams") -> None:
    lang: str | None = traits.get_language(params["markanm_locale"], traits.all_locale)
    search_url: str = (
        base_url
        + "/s/{blend_query}/?page={pageno}"
...
def response(resp: "SXNG_Response") -> SourceResults:
    ...
    if domain_is_seized(dom):
        raise BlendException(f"zlibrary domain is seized: {base_url}")
...
def fetch_traits(engine_traits: EngineTraits) -> None:
    ...
    from blend_core.locales import language_tag
