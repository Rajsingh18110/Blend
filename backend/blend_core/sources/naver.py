...
from blend_core.exceptions import BlendEngineAPIException, BlendEngineXPathException
...
def init(_):
    if naver_category not in ('general', 'images', 'news', 'videos'):
        raise BlendEngineAPIException(f"Unsupported category: {naver_category}")
...
def parse_general(data):
    ...
    for item in eval_xpath_list(dom, "//ul[contains(@class, 'lst_total')]/li[contains(@class, 'bx')]"):
        ...
        except (ValueError, TypeError, BlendEngineXPathException):
            pass
