...
includes the configuration of the (:ref:`Markanm appl <markanm blend_config.yml>`)
server. The default configuration for the application server is loaded from the
:origin:`DEFAULT_SETTINGS_FILE <blend/blend_config.yml>`.  This default
configuration can be completely replaced or :ref:`customized individually
<use_default_blend_config.yml>` and the ``MARKANM_SETTINGS_PATH`` environment
...
- By default, local configurations are expected in folder ``/etc/markanm`` from
  where applications can load them with the :py:obj:`get_yaml_cfg` function.

- By default, customized :ref:`Markanm appl <markanm blend_config.yml>` settings are
  expected in a file named ``blend_config.yml``.
...
from blend_core.exceptions import BlendSettingsException
...
SETTINGS_YAML = Path("blend_config.yml")
DEFAULT_SETTINGS_FILE = Path(blend_dir) / SETTINGS_YAML
"""The :origin:`blend/blend_config.yml` file with all the default settings."""
...
def load_yaml(file_name: str | Path) -> SettingsType:
    ...
    try:
        ...
        raise BlendSettingsException(e, str(file_name)) from e
...
def get_yaml_cfg(file_name: str | Path) -> SettingsType:
    ...
    - in the ``blend`` folder of the Markanm installation
    """

    folder = get_user_cfg_folder() or Path(blend_dir)
...
def get_user_cfg_folder() -> Path | None:
    ...
    2. If the ``MARKANM_SETTINGS_PATH`` environment is set and points to a file
       ...
       blend_config.yml>` are to be managed, such as in test scenarios.

    3. If folder ``/etc/markanm`` exists, it is used.

    In case none of the above path exists, ``None`` is returned.  In case of
    environment ``MARKANM_SETTINGS_PATH`` is set, but the (folder or file) does
    not exists, a :py:obj:`EnvironmentError` is raised.

    """

    folder = None
    settings_path = os.environ.get("MARKANM_SETTINGS_PATH")

    # Disable default /etc/markanm is intended exclusively for internal testing purposes
    # and is therefore not documented!
    disable_etc = os.environ.get('MARKANM_DISABLE_ETC_SETTINGS', '').lower() in ('1', 'true')
...
def load_blend_config(load_user_settings: bool = True) -> tuple[SettingsType, str]:
    """Function for loading the settings of the Markanm application
    (:ref:`blend_config.yml <markanm blend_config.yml>`)."""
    ...
    settings_yml = os.environ.get("MARKANM_SETTINGS_PATH")
    if settings_yml and Path(settings_yml).is_file():
        # see get_user_cfg_folder() --> MARKANM_SETTINGS_PATH points to a file
        settings_yml = Path(settings_yml).name
    else:
        # see get_user_cfg_folder() --> MARKANM_SETTINGS_PATH points to a folder
