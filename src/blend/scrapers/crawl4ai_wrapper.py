...
class Crawl4AIWrapper:
    ...
    async def extract_many(self, urls: list[str], limit: int = 5) -> list[Dict[str, Any]]:
        """
        Extract multiple URLs concurrently, up to the given limit.
        """
        urls_to_process = urls[:limit]
        tasks = [self.extract(url) for url in urls_to_process]
        results = await asyncio.gather(*tasks, return_exceptions=True)
        
        final_results = []
        for i, res in enumerate(results):
            if isinstance(res, Exception):
                final_results.append({"markdown": "", "success": False, "url": urls_to_process[i], "error": str(res)})
            else:
