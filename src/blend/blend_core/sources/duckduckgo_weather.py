...
def _weather_data(location: weather.GeoLocation, data: dict[str, t.Any]):
    ...
    return SourceResults.types.WeatherAnswer.Item(
        location=location,
...
def request(query: str, params: dict[str, t.Any]):
    ...
    eng_region = traits.get_region(params['markanm_locale'], traits.all_locale)
    eng_lang = get_ddg_lang(traits, params['markanm_locale'])
...
def response(resp: BlendResponse):
    ...
    weather_answer = SourceResults.types.WeatherAnswer(
