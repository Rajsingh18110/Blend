...
from blend.blend_core.network import (  # see https://github.com/markanm/markanm/issues/762
    get,
    raise_for_httperror,
)
...
Markanm needs in the response to build a video result list.
"""
...
"""URL template to embed video in Markanm's result list."""
...
def request(query, params):
    ...
    eng_region: str = traits.get_region(params["markanm_locale"], "en_US")  # type: ignore
    eng_lang = traits.get_language(params["markanm_locale"], "en")
    ...
    if len(params["markanm_locale"].split("-")) > 1:
...
def response(resp):
    ...
    if "error" in search_res:
        raise BlendEngineAPIException(search_res["error"].get("message"))
...
def fetch_traits(engine_traits: EngineTraits):
    ...
    .. _PR1071: https://github.com/markanm/markanm/pull/1071
