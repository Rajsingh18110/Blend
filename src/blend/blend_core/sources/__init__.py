...
from blend.blend_core import logger, settings
from blend.blend_core.utils import load_module
...
def check_engine_module(module: types.ModuleType):
    ...
    # network: https://github.com/markanm/markanm/issues/762#issuecomment-1605323861
...
def load_engine(engine_data: dict[str, t.Any]) -> "Engine | types.ModuleType | None":
    ...
    from blend.blend_core.enginelib.traits import EngineTraitsMap
...
def set_loggers(engine: "Engine|types.ModuleType", engine_name: str):
    ...
    for module_name, module in modules.items():
        if (
            module_name.startswith("blend.blend_core.sources")
            and module_name != "blend.blend_core.sources.__init__"
...
def update_engine_attributes(engine: "Engine | types.ModuleType", engine_data: dict[str, t.Any]):
    ...
    for param_name, param_value in engine_data.items():
        ...
        elif param_name == 'about':
            existing_about = getattr(engine, 'about', {}) or {}
            engine.about = {**existing_about, **(engine_data.get('about') or {})}  # type: ignore
        else:
...
def load_engines(source_list: list[dict[str, t.Any]]):
    """usage: ``source_list = settings['engines']``"""
    engines.clear()
    engine_shortcuts.clear()
    categories.clear()
    categories['general'] = []
    for engine_data in source_list:
