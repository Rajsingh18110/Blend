...
.. _DuckDuckGo Bot Detection Research & Solution:
   https://github.com/ggfevans/markanm/blob/mod-sidecar-harvester/docs/ddg-bot-detection-research.md
...
from blend.blend_core.external_bang import EXTERNAL_BANGS, get_node  # type: ignore
...
def get_ddg_lang(
    ...
    It might confuse, but the ``l`` value of the cookie is what Markanm calls
    the *region*:
...
def request(query: str, params: "OnlineParams") -> None:
    ...
    if params["pageno"] == 1:
        ...
        if vqd:
            ...
            # reputation of the Markanm IP and DDG starts to activate CAPTCHAs.
            # set suspend time to zero is OK --> ddg does not block the IP
            raise BlendEngineCaptchaException(
                suspended_time=0,
                message=f"VQD missed (page: {params['pageno']}, locale: {params['markanm_locale']})",
            )

        if params["markanm_locale"].startswith("zh"):
...
def response(resp: "BlendResponse") -> SourceResults:
    ...
    if is_ddg_captcha(doc):
        # set suspend time to zero is OK --> ddg does not block the IP
        raise BlendEngineCaptchaException(suspended_time=0, message=f"CAPTCHA ({params['data'].get('kl')})")
...
def fetch_traits(engine_traits: EngineTraits):
    ...
    Markanm's ``all`` locale maps DuckDuckGo's "All regions" (``wt-wt``).
    DuckDuckGo's language "Browsers preferred language" (``wt_WT``) makes no
    sense in a Markanm request since Markanm's ``all`` will not add a
    ...
    from blend.blend_core.utils import js_obj_str_to_python
