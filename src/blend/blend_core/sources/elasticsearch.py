...
""".. sidebar:: info
   ...
   - :origin:`elasticsearch.py <blend_core/sources/elasticsearch.py>`
...
def _match_query(query):
    ...
    Markanm format: "key:value" e.g. city:berlin
...
def _term_query(query):
    ...
    blend format: "key:value" e.g. city:berlin
...
def _terms_query(query):
    ...
    blend format: "key:value1,value2" e.g. city:berlin,paris
...
def response(resp: BlendResponse) -> SourceResults:
    ...
    if 'error' in resp_json:
        raise BlendEngineAPIException(resp_json["error"])
