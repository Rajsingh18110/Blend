...
video streams Markanm can't use the video template for this and if Markanm can't
...
from blend.blend_core import locales
from blend.blend_core.network import get
from blend.blend_core.utils import gen_useragent, html_to_text, parse_duration_string
...
def _get_request_id(query, params):
    ...
    if params['markanm_locale'] != 'all':
        l = locales.get_locale(params['markanm_locale'])

        # Presearch narrows down the search by region.  In Markanm when the user
        # does not set a region (e.g. 'en-CA' / canada) we cannot hand over a region.

        # We could possibly use blend_core.locales.get_official_locales to determine
...
def _fix_title(title, url):
    ...
    Transforming "translate.blend.co.in<em>blend</em> Translate" into "blend Translate"
    """
    parsed_url = urlparse(url)
    domain = parsed_url.netloc
    title = html_to_text(title)
    # Fixes issue where domain would show up in the title
    # translate.blend.co.inblend Translate -> blend Translate
    if (
...
def parse_blend_query(json_results):
    results = []
    if not json_results:
        return results
...
def response(resp):
    ...
    if search_type == 'search':
        results = parse_blend_query(json_resp.get('results', {}))
        ...
        # a video and not to a video stream --> Markanm can't use the video template.
