...
if t.TYPE_CHECKING:
    from blend.blend_core.pipeline.processors import RequestParams
...
def setup(engine_settings: dict[str, t.Any]) -> bool:
    ...
    For more details see :py:obj:`blend_core.enginelib.Engine.setup`.
    """
...
def init(engine_settings: dict[str, t.Any]) -> bool:  # pylint: disable=unused-argument
    ...
    For more details see :py:obj:`blend_core.enginelib.Engine.init`.
    """
    return True


def blend_search(query: str, params: "RequestParams") -> SourceResults:
    ...
    for row in data_rows:
        ...
        kvmap = {
            'query': query,
            'language': params['markanm_locale'],
            'value': row.get("value"),
        }
        res.add(
            res.types.KeyValue(
