...
This implementation is used by different qwant engines in the :ref:`blend_config.yml
<settings engines>`:
...
from blend.blend_core.network import raise_for_httperror
...
def request(query, params):
    ...
    q_locale = traits.get_region(params["markanm_locale"], default="en_US")
...
def parse_web_api(resp):
    ...
    if search_results.get("status") != "success":
        ...
        raise BlendEngineAPIException(f"{msg} ({error_code})")
...
def fetch_traits(engine_traits: EngineTraits):
    ...
    from blend.blend_core.utils import extr
