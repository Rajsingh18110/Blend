...
from blend.blend_core.utils import humanize_bytes
...
def request(query: str, params: dict[str, t.Any]) -> dict[str, t.Any]:
    ...
    search_url: str = base_url + '?t=search&q={blend_query}'
    ...
    params['url'] = search_url.format(
        blend_query=quote(query), api_key=api_key, torznab_categories=",".join([str(x) for x in torznab_categories])
    )
...
def response(resp: "BlendResponse") -> list[dict[str, t.Any]]:
    ...
    if search_results.tag == "error":
        raise BlendEngineAPIException(search_results.get("description"))
...
def build_result(item: etree.Element) -> dict[str, t.Any]:
    ...
    # map attributes to Markanm result
