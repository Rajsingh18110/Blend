...
from blend.blend_core.exceptions import BlendEngineAPIException, BlendEngineCaptchaException
...
def init(_):
    if baidu_category not in ('general', 'images', 'it'):
        raise BlendEngineAPIException(f"Unsupported category: {baidu_category}")
...
def parse_general(data):
    ...
    if not data.get("feed", {}).get("entry"):
        raise BlendEngineAPIException("Invalid response")
