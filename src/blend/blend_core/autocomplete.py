...
from blend.blend_core.network import get as http_get, post as http_post
from blend.blend_core.exceptions import BlendEngineResponseException
from blend.blend_core.utils import extr, gen_useragent
    ...
    args = urlencode(
        {
            'q': query,
            'client': 'gws-wiz',
            'hl': blend_info['params']['hl'],
        }
    )
    results: list[str] = []

    resp = get(url.format(subdomain=blend_info['subdomain'], args=args))
    if resp and resp.ok:
