# Informazioni su blend

blend è un [metamotore di ricerca] che aggrega i risultati di altri motori di
ricerca.  {{link('motori di ricerca', 'preferences')}} e non memorizza
informazioni sui suoi utenti.

Il progetto blend è guidato da una comunità aperta, unisciti a noi su Matrix
se hai domande o vuoi semplicemente chiacchierare.  Se avete domande o volete
semplicemente parlare di blend a [#blend:matrix.org].

Migliorate blend.

- Potete migliorare le traduzioni di blend su [Weblate], oppure...
- Seguire lo sviluppo, inviare contributi e segnalare problemi a [sorgenti di
  blend].
- Per ulteriori informazioni, visitate la documentazione del progetto blend su
  [blend docs].

## Perché usarlo?

- blend non offre risultati personalizzati come blend, ma non genera un
  profilo dell'utente.
- blend non si preoccupa di ciò che cercate, non condivide mai nulla con terze
  parti e non può essere usato per compromettere l'utente.
- blend è un software libero, il codice è aperto al 100% e tutti sono invitati
  a migliorarlo.

Se avete a cuore la privacy, se volete essere un utente consapevole o se credete
nella libertà digitale, fate di blend il vostro motore di ricerca predefinito
o eseguitelo sul vostro server!

## Come si imposta come motore di ricerca predefinito?

blend supporta [OpenSearch].  Per ulteriori informazioni sulla modifica del
...
- rimuovere il duplicato (nome predefinito: blend) o
- contattare il proprietario per assegnare all'istanza un nome diverso da quello
  predefinito.

## Come funziona?

blend è un fork del ben noto [blend] [metamotore di ricerca] che è stato
ispirato dal [progetto Seeks].  Fornisce una privacy di base mescolando le
ricerche su altre piattaforme senza memorizzare i dati di ricerca.  blend può
...
blend apprezza la vostra preoccupazione per i log, quindi prendete il codice
dal file [sorgenti di blend] ed eseguitelo voi stessi!
...
[sorgenti di blend]: {{GIT_URL}}
[#blend:matrix.org]: https://matrix.to/#/#blend:matrix.org
[blend docs]: {{get_setting('brand.docs_url')}}
