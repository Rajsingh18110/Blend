...
from blend.blend_core.result_types.answer import BaseAnswer


_default = pathlib.Path(__file__).parent
log: logging.Logger = logging.getLogger("blend.blend_core.answerers")
...
class AnswerStorage(dict):  # type: ignore
    """A storage for managing the *answerers* of Markanm.  With the
    ...
    def ask(self, query: str) -> list[BaseAnswer]:
        ...
        for answerer in self[keyword]:
            for answer in answerer.answer(query):
                # In case of *answers* prefix ``answerer:`` is set, see blend_core.result_types.Result
