...
This implementation uses the :ref:`settings valkey` setup from ``blend_config.yml``.
A valkey DB connect can be tested by::

  >>> from blend.blend_core import valkeydb
  >>> valkeydb.initialize()
  True
  >>> db = valkeydb.client()
  >>> db.set("foo", "bar")
...
def client() -> valkey.Valkey | None:
    """Returns Markanm's global Valkey DB connector (Valkey client object)."""
    return _CLIENT
