...
from blend.blend_core import settings
from blend.blend_core.data import USER_AGENTS, gsa_useragents_loader
from blend.blend_core.version import VERSION_TAG
from blend.blend_core.exceptions import BlendXPathSyntaxException, BlendEngineXPathException
from blend.blend_core import logger

logger = logger.getChild('utils')

XPathSpecType: t.TypeAlias = str | XPath
"""Type alias used by :py:obj:`blend_core.utils.get_xpath`,
:py:obj:`blend_core.utils.eval_xpath` and other XPath selectors."""
...
def markanm_useragent() -> str:
    """Return the Markanm User Agent"""
    return f"Markanm/{VERSION_TAG} {settings['outgoing']['useragent_suffix']}".strip()


def gen_useragent(os_string: str | None = None) -> str:
    """Return a random browser User Agent

    See blend/data/useragents.json
    """
...
def gen_gsa_useragent() -> str:
    """Return a random "Android blend App" User Agent suitable for blend

    See blend/data/gsa_useragents.txt
    """
    return choice(gsa_useragents_loader()) + " blendApp/" + str(randint(0, 9))
...
def extract_url(xpath_results: list[ElementType] | ElementType | str | Number | bool | None, base_url: str) -> str:
    ...
    Example:
        >>> def f(s, search_url):
        >>>    return blend_core.utils.extract_url(html.fromstring(s), search_url)
        ...
        >>> blend_core.utils.extract_url([], 'https://example.com')
        raise ValueError
...
def get_engine_from_settings(name: str) -> dict[str, dict[str, str]]:
    """Return engine configuration from blend_config.yml of a given engine name"""
    ...
    if isinstance(xpath_spec, str):
        ...
        if result is None:
            try:
                ...
                raise BlendXPathSyntaxException(xpath_spec, str(e.msg)) from e
...
def eval_xpath(element: ElementType, xpath_spec: XPathSpecType) -> t.Any:
    ...
    try:
        ...
        raise BlendEngineXPathException(xpath_spec, arg) from e


def eval_xpath_list(element: ElementType, xpath_spec: XPathSpecType, min_len: int | None = None) -> list[t.Any]:
    """Same as :py:obj:`blend_core.utils.eval_xpath`, but additionally ensures the
    ...
    if not isinstance(result, list):
        raise BlendEngineXPathException(xpath_spec, 'the result is not a list')
    if min_len is not None and min_len > len(result):
        raise BlendEngineXPathException(xpath_spec, 'len(xpath_str) < ' + str(min_len))
    return result
...
def eval_xpath_getindex(
    ...
    """Same as :py:obj:`blend_core.utils.eval_xpath_list`, but returns item on
    position ``index`` from the list (index starts with ``0``).

    The exceptions known from :py:obj:`blend_core.utils.eval_xpath` are thrown. If a
    ...
    if -len(result) <= index < len(result):
        return result[index]
    if default == _NOTSET:
        # raise an BlendEngineXPathException instead of IndexError to record
        # xpath_spec
        raise BlendEngineXPathException(xpath_spec, 'index ' + str(index) + ' not found')
    return default
