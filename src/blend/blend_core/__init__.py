...
blend_dir: str = abspath(dirname(__file__))
blend_parent_dir: str = abspath(dirname(dirname(__file__)))
...
def init_settings():
    """Initialize global ``settings`` and ``blend_debug`` variables and
    ``logger`` from ``MARKANM_SETTINGS_PATH``.
    """

    # pylint: disable=import-outside-toplevel
    from blend.blend_core import settings_loader
    from blend.blend_core.settings_defaults import SCHEMA, apply_schema

    global settings, blend_debug  # pylint: disable=global-variable-not-assigned

    cfg, msg = settings_loader.load_blend_config(load_user_settings=True)
    cfg = cfg or {}
    ...
    blend_debug = get_setting("general.debug")
    if blend_debug:
    ...
    if settings['server']['public_instance']:
        logger.warning(
            ...
            "see https://docs.markanm.org/admin/blend_core.limiter.html"
        )
...
def _logging_config_debug():
    ...
    log_level = os.environ.get('MARKANM_DEBUG_LOG_LEVEL', 'DEBUG')
...
# Compatibility alias for modules that still import the old engine registry name.
from blend.blend_core import sources as engines  # noqa: E402  pylint: disable=wrong-import-position
