...
from blend.blend_core import logger as log
from blend import blend_core
import blend.blend_core.sources
from blend.blend_core.metrics import histogram_observe, counter_add
from blend.blend_core.result_types import Result, LegacyResult, MainResult
from blend.blend_core.result_types.answer import AnswerSet, BaseAnswer
...
def calculate_score(
    ...
    for result_engine in result['engines']:
        if hasattr(blend_core.sources.engines.get(result_engine), 'weight'):
            weight *= float(blend_core.sources.engines[result_engine].weight)
...
class BlendResults:
    ...
    def extend(
        ...
        if engine_name in blend_core.sources.engines:
            eng = blend_core.sources.engines[engine_name]
    ...
    def get_ordered_results(self) -> list[MainResult | LegacyResult]:
        ...
        for res in results:
            ...
            engine = blend_core.sources.engines.get(res.engine or "")
            if engine:
    ...
    def number_of_results(self) -> int:
        ...
        if not self._closed:
            log.error("call to BlendResults.number_of_results before BlendResults.close")
            return 0
    ...
    def add_unresponsive_engine(self, engine_name: str, error_type: str, suspended: bool = False):
        with self._lock:
            if self._closed:
                log.error("call to BlendResults.add_unresponsive_engine after BlendResults.close")
                return
            if blend_core.sources.engines[engine_name].display_error_messages:
    ...
    def add_timing(self, engine_name: str, engine_time: float, page_load_time: float):
        with self._lock:
            if self._closed:
                log.error("call to BlendResults.add_timing after BlendResults.close")
                return
    ...
    def get_timings(self) -> list[Timing]:
        with self._lock:
            if not self._closed:
                log.error("call to BlendResults.get_timings before BlendResults.close")
                return []
            return self.timings
...
def merge_two_infoboxes(origin: LegacyResult, other: LegacyResult):
    ...
    # pylint: disable=too-many-branches
    weight1 = getattr(blend_core.sources.engines[origin.engine], "weight", 1)
    weight2 = getattr(blend_core.sources.engines[other.engine], "weight", 1)
