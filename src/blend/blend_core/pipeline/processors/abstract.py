...
from blend.blend_core import get_setting
from blend.blend_core import logger
from blend.blend_core.sources import engines
from blend.blend_core.network import get_time_for_thread, get_network
from blend.blend_core.metrics import histogram_observe, counter_inc, count_exception, count_error
from blend.blend_core.exceptions import BlendEngineAccessDeniedException
from blend.blend_core.utils import get_engine_from_settings

if t.TYPE_CHECKING:
    import types
    from blend.blend_core.enginelib import Engine
...
logger = logger.getChild("blend.blend_core.pipeline.processor")
...
class EngineProcessor(ABC):
    ...
    def handle_exception(
        ...
        result_pool.add_unresponsive_engine(self.engine.name, error_message)
        # metrics
        ...
        if suspend:
            suspended_time = None
            if isinstance(exception_or_message, BlendEngineAccessDeniedException):
    ...
    def _extend_container_basic(
        ...
        # update result_pool
        result_pool.extend(self.engine.name, search_results)
        engine_time = default_timer() - start_time
        page_load_time = get_time_for_thread()
        result_pool.add_timing(self.engine.name, engine_time, page_load_time)
        # metrics
    ...
    def extend_container(
        ...
        if getattr(threading.current_thread(), '_timeout', False):
            ...
            self.handle_exception(result_pool, 'timeout', False)
        else:
            # check if the engine accepted the request
            if search_results is not None:
                self._extend_container_basic(result_pool, start_time, search_results)
            self.suspended_status.resume()

    def extend_container_if_suspended(self, result_pool: "BlendResults") -> bool:
        if self.suspended_status.is_suspended:
            result_pool.add_unresponsive_engine(
                self.engine.name, self.suspended_status.suspend_reason, suspended=True
            )
            return True
        return False

    def get_params(self, blend_query: "BlendQuery", engine_category: str) -> RequestParams | None:
        ...
        if blend_query.pageno > 1 and not self.engine.paging:
            return None

        # if max page is reached, skip
        max_page = self.engine.max_page or get_setting("search.max_page")
        if max_page and max_page < blend_query.pageno:
            return None

        # if time_range is not supported, skip
        if blend_query.time_range and not self.engine.time_range_support:
            return None
        ...
        params: RequestParams = {
            ...
            "safesearch": blend_query.safesearch,
            "time_range": blend_query.time_range,
            "engine_data": blend_query.engine_data.get(self.engine.name, {}),
            "markanm_locale": blend_query.lang,
        }

        # params["language"] is deprecated --> use params["markanm_locale"]
        #
        ...
        if hasattr(self.engine, "language") and self.engine.language:
            ...
            params["language"] = blend_query.lang  # pyright: ignore[reportGeneralTypeIssues]
