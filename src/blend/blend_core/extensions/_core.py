...
if t.TYPE_CHECKING:
    ...
    from blend.blend_core.result_types import Result, SourceResults, LegacyResult  # pyright: ignore[reportPrivateLocalImportUsage]
    import flask

log: logging.Logger = logging.getLogger("blend.blend_core.extensions")
...
class Plugin(abc.ABC):
    ...
    def on_result(self, request: BlendRequest, search: "SearchWithPlugins", result: "Result") -> bool:
        ...
        .. hint::
           ...
           If :py:obj:`Result.url <blend_core.result_types._base.Result.url>` is modified,
           :py:obj:`Result.parsed_url <blend_core.result_types._base.Result.parsed_url>` must
...
class PluginStorage:
    """A storage for managing the *plugins* of Markanm."""
    ...
    def load_blend_config(self, cfg: dict[str, dict[str, t.Any]]):
        """Load plugins configured in Markanm's settings :ref:`settings
        plugins`."""
    ...
    def post_search(self, request: BlendRequest, search: "SearchWithPlugins") -> None:
        ...
        keyword = None
        for keyword in search.blend_query.query.split():
            if keyword:
                break
        ...
        for plugin in [p for p in self.plugin_list if p.id in search.user_plugins]:
            ...
            # In case of *plugins* prefix ``plugin:`` is set, see blend_core.result_types.Result
            search.result_pool.extend(f"plugin: {plugin.id}", results)
