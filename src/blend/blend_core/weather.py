...
from blend.blend_core import network
from blend.blend_core.cache import ExpireCache, ExpireCacheCfg
from blend.blend_core.extended_types import blend_request
from blend.blend_core.wikidata_units import convert_to_si, convert_from_si
...
def _get_blend_locale_tag() -> str:
    ...
    #
    # In principle, Markanm only has two possible parameters for determining
    ...
    # probably never be available in Markanm (privacy critical).
    #
    ...
    from blend.blend_core import query
    from blend.blend_core.preferences import ClientPref

    query = query.RawTextQuery(blend_request.form.get("q", ""), [])
    if query.languages and query.languages[0] not in ["all", "auto"]:
        return query.languages[0]

    search_lang = blend_request.form.get("language")
...
class WindSpeed(msgspec.Struct, kw_only=True):
    ...
    .. hint::
       ...
       Working with unit ``Bft`` (:py:obj:`blend_core.wikidata_units.Beaufort`) will
