...
from blend.blend_core import logger, blend_debug
...
# requests compatibility when reading proxy settings from blend_config.yml
...
class Network:
    ...
    async def get_client(self, verify: bool | None = None, max_redirects: int | None = None) -> httpx.AsyncClient:
        ...
        hook_log_response = self.log_response if blend_debug else None
    ...
    try:
        ...
        if loop:
            ...
            try:
                ...
                if os.environ.get("BLEND_EMBEDDED_BACKEND", "").lower() not in ("1", "true"):
                    raise
    finally:
        NETWORKS.clear()
