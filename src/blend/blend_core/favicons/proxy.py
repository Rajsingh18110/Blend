...
from blend.blend_core.webutils import new_hmac, is_hmac_of
from blend.blend_core.exceptions import BlendEngineResponseException
...
def _initial_resolver_map():
    ...
    if name:
        ...
        if func:
            d = {name: f"blend.blend_core.favicons.resolvers.{func.__name__}"}
    return d
...
class FaviconProxyConfig(msgspec.Struct):
    ...
    :py:obj:`blend_core.favicons.resolver` are available by default."""
...
def favicon_proxy():
    """REST API of Markanm's favicon proxy service
    ...
    """
    authority = blend_request.args.get('authority')
    ...
    if not is_hmac_of(
        CFG.secret_key,
        authority.encode(),
        blend_request.args.get('h', ''),
    ):
        return '', 400

    resolver = blend_request.preferences.get_value('favicon_resolver')  # type: ignore
    ...
    theme = blend_request.preferences.get_value("theme")  # type: ignore
...
def favicon_url(authority: str) -> str:
    """Function to generate the image URL used for favicons in Markanm's result
