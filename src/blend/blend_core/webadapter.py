...
from blend.blend_core.webutils import VALID_LANGUAGE_CODE
from blend.blend_core.query import RawTextQuery
from blend.blend_core.sources import categories, engines
from blend.blend_core.pipeline.models import BlendQuery, SourceRef
from blend.blend_core.preferences import Preferences, is_locked


# remove duplicate queries.
# HINT: does not fix "!music !soundcloud", because the categories are 'none' and 'music'
def deduplicate_engineref_list(engineref_list: List[SourceRef]) -> List[SourceRef]:
    engineref_dict = {q.category + '|' + q.name: q for q in engineref_list}
    return list(engineref_dict.values())


def validate_engineref_list(
    engineref_list: List[SourceRef], preferences: Preferences
) -> Tuple[List[SourceRef], List[SourceRef], List[SourceRef]]:
    """Validate query_engines according to the preferences

    Returns:
        List[SourceRef]: list of existing engines with a validated token
        List[SourceRef]: list of unknown engine
        List[SourceRef]: list of engine with invalid token according to the preferences
    """
    valid = []
    unknown = []
    no_token = []
...
def parse_pageno(form: Dict[str, str]) -> int:
    ...
    if not pageno_param.isdigit() or int(pageno_param) < 1:
        raise BlendParameterException('pageno', pageno_param)
...
def parse_lang(preferences: Preferences, form: Dict[str, str], raw_text_query: RawTextQuery) -> str:
    ...
    if not VALID_LANGUAGE_CODE.match(query_lang) and query_lang != 'auto':
        raise BlendParameterException('language', query_lang)
...
def parse_safesearch(preferences: Preferences, form: Dict[str, str]) -> int:
    ...
    if 'safesearch' in form:
        ...
        if not query_safesearch.isdigit():
            raise BlendParameterException('safesearch', query_safesearch)
...
def parse_time_range(form: Dict[str, str]) -> Optional[str]:
    ...
    if query_time_range not in (None, 'day', 'week', 'month', 'year'):
        raise BlendParameterException('time_range', query_time_range)
...
def parse_timeout(form: Dict[str, str], raw_text_query: RawTextQuery) -> Optional[float]:
    ...
    try:
        ...
        raise BlendParameterException('timeout_limit', timeout_limit) from e
...
def parse_generic(preferences: Preferences, form: Dict[str, str], disabled_engines: List[str]) -> List[SourceRef]:
    query_engineref_list = []
    query_categories = []

    # set categories/engines
    explicit_source_list = False
...
def get_blend_query_from_webapp(
    preferences: Preferences, form: Dict[str, str]
) -> Tuple[BlendQuery, RawTextQuery, List[SourceRef], List[SourceRef], str]:
    """Assemble data from preferences and request.form (from the HTML form) needed
    in a search query.

    The returned tuple consists of:

    1. instance of :py:obj:`blend_core.pipeline.BlendQuery`
    2. instance of :py:obj:`blend_core.query.RawTextQuery`
    3. list of :py:obj:`blend_core.pipeline.SourceRef` instances
    ...
    if not form.get('q'):
        raise BlendParameterException('q', '')
