...
"""
Markanm’s locale data
=====================

The variables :py:obj:`RTL_LOCALES` and :py:obj:`LOCALE_NAMES` are loaded from
:origin:`blend/data/locales.json` / see :py:obj:`locales_initialize` and
...
.. hint::
   ...
   :origin:`blend/data/locales.json` needs to be rebuild::

     ./manage data.locales

Markanm's locale codes
======================

.. automodule:: blend_core.blend_locales
   :members:


Markanm’s locale implementations
================================
"""
...
"""Additional languages Markanm has translations for but not supported by
...
def localeselector():
    ...
    if has_request_context():
        value: str = blend_request.preferences.get_value('locale')
        if value:
            locale = value

    # first, set the language that is not supported by babel
    if locale in ADDITIONAL_TRANSLATIONS:
        blend_request.form['use-translation'] = locale
...
def get_translations():
    ...
    if has_request_context():
        use_translation = blend_request.form.get('use-translation')
...
def get_translation_locales() -> list[str]:
    ...
    generated from the translation folders in :origin:`blend/translations`"""
    ...
    tr_locales: list[str] = []
    for folder in (Path(blend_dir) / 'translations').iterdir():
...
def locales_initialize():
    """Initialize locales environment of the Markanm session.
...
def region_tag(locale: babel.Locale) -> str:
    """Returns Markanm's region tag from the locale (e.g. zh-TW , en-US)."""
...
def get_engine_locale(markanm_locale: str, engine_locales: dict[str, str], default: str | None = None) -> str | None:
    """Return engine's language (aka locale) string that best fits to argument
    ``markanm_locale``.

    Argument ``engine_locales`` is a python dict that maps *Markanm locales* to
    corresponding *engine locales*::

      <engine>: {
          # Markanm string : engine-string
          'ca-ES'          : 'ca_ES',
          'fr-BE'          : 'fr_BE',
          'fr-CA'          : 'fr_CA',
          'fr-CH'          : 'fr_CH',
    ...
    .. hint::
       ...
       The *Markanm locale* string has to be known by babel!
    ...
    engine_locale = engine_locales.get(markanm_locale)
    ...
    try:
        locale = babel.Locale.parse(markanm_locale, sep='-')
    except babel.core.UnknownLocaleError:
        try:
            locale = babel.Locale.parse(markanm_locale.split('-')[0])
        except babel.core.UnknownLocaleError:
            return default

    markanm_lang = language_tag(locale)
    engine_locale = engine_locales.get(markanm_lang)
    if engine_locale is not None:
        # There was a 1:1 mapping (e.g. "zh-HK --> zh_Hant" or "zh-CN --> zh_Hans")
        return engine_locale

    # Markanm's selected locale is not supported by the engine ..
    ...
    if locale.territory:
        ...
        for official_language in babel.languages.get_official_languages(locale.territory, de_facto=True):
            markanm_locale = official_language + '-' + locale.territory
            engine_locale = engine_locales.get(markanm_locale)
            if engine_locale is not None:
                return engine_locale
    ...
    # Now lets have a look if the markanm_lang (the language selected by the
    # user) is a official language in other territories.  If so, check if
    # engine does support the markanm_lang in this other territory.
    ...
    if locale.language:
        ...
        if terr_lang_dict.get(territory):
            markanm_locale = locale.language + '-' + territory
            engine_locale = engine_locales.get(markanm_locale)
            if engine_locale is not None:
                return engine_locale
...
def match_locale(markanm_locale: str, locale_tag_list: list[str], fallback: str | None = None) -> str | None:
    """Return tag from ``locale_tag_list`` that best fits to ``markanm_locale``.

    :param str markanm_locale: Markanm's internal representation of locale (de,
    ...
    .. hint::
       ...
       The *Markanm locale* string and the members of ``locale_tag_list`` has to
    ...
    if not markanm_locale:
        return fallback

    locale = get_locale(markanm_locale)
    if locale is None:
        return fallback

    # normalize to a Markanm locale that can be passed to get_engine_locale

    markanm_locale = language_tag(locale)
    if locale.territory:
        markanm_locale = region_tag(locale)
    ...
    return get_engine_locale(markanm_locale, engine_locales, default=fallback)
...
def build_engine_locales(tag_list: list[str]) -> dict[str, str]:
    ...
    of Markanm:
      ...
      Markanm only distinguishes between **search languages** and **search
      regions**, by adding the *script-tags*, languages with *script-tags* can
      be assigned to the **regions** that Markanm supports.
