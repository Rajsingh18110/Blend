...
# fallback values
# if there is blend_core.version_frozen module, and it is not possible to get the git tag
...
def get_git_version() -> tuple[str, str, str]:
    ...
    # add "+dirty" suffix if there are uncommitted changes except blend/blend_config.yml
    try:
        subprocess_run("git diff --quiet -- . ':!blend/blend_config.yml' ':!utils/brand.env'")
...
try:
    vf = importlib.import_module('blend.blend_core.version_frozen')
...
if __name__ == "__main__":
    ...
    if len(sys.argv) >= 2 and sys.argv[1] == "freeze":
        ...
        python_code = f"""# SPDX-License-Identifier: Apache-2.0
# pylint: disable=missing-module-docstring
# this file is generated automatically by blend/version.py
...
"""
        ...
        # usage: eval "$(python -m blend_core.version)"
        shell_code = f"""
VERSION_STRING="{VERSION_STRING}"
VERSION_TAG="{VERSION_TAG}"
DOCKER_TAG="{DOCKER_TAG}"
