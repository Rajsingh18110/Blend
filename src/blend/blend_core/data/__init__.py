...
def __getattr__(name: str) -> t.Any:
    ...
    log.debug("init blend_core.data.%s", name)
...
def ahmia_blacklist_loader() -> list[str]:
    ...
    names.  The MD5 values are fetched by::
      ...
      markanm_extra/update/update_ahmia_blacklist.py

    This function is used by :py:mod:`blend_core.extensions.ahmia_filter`.
...
def gsa_useragents_loader() -> list[str]:
    ...
    suitable for blend.  The user agents are fetched by::

      markanm_extra/update/update_gsa_useragents.py

    This function is used by :py:mod:`blend_core.sources.blend`.
    ...
    log.debug("init blend_core.data.%s", "GSA_USER_AGENTS")
