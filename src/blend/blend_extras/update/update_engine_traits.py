...
"""Update :py:obj:`blend_core.enginelib.traits.EngineTraitsMap` and :origin:`blend/languages.py`

:py:obj:`blend_core.enginelib.traits.EngineTraitsMap.ENGINE_TRAITS_FILE`:
  Persistence of engines traits, fetched from the engines.

:origin:`blend/languages.py`
  Is generated  from intersecting each engine's supported traits.

The script :origin:`markanm_extra/update/update_engine_traits.py` is called in
...
from blend.blend_core import settings, blend_dir
...
# Output files.
languages_file = Path(blend_dir) / 'blend_locales.py'
languages_file_header = """\
# SPDX-License-Identifier: Apache-2.0
'''List of Markanm's locale codes used for the search language/region.
...
0. Markanm's internal locale tag (a language or region tag)
...
4. Unicode flag (emoji) that fits to Markanm's internal region tag. Languages
